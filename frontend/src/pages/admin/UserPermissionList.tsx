...
import { deleteUserPermission, fetchUserPermissionList, UserPermissionListRow } from "../../api/userPermissionApi";
...
export default function UserPermissionList() {
  ...
  const [rows, setRows] = useState<UserPermissionListRow[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchUserPermissionList({ search, start, length: requestLength });
      setRows(res.data);
      setTotal(res.recordsFiltered);
    } catch {
      setRows([]);
      setTotal(0);
      await showErrorAlert("Failed to load user permission list.");
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (unique_id: string) => {
    const confirmed = await showConfirmAlert("Delete this user permission record?");
    if (!confirmed) return;

    try {
      const res = await deleteUserPermission(unique_id);
      if (res?.status === false) throw new Error(res?.error || "Failed to delete record.");
      await showSuccessAlert("User permission deleted successfully.");
      loadData();
    } catch (err) {
  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(204,217,177,0.22),_transparent_30%),linear-gradient(180deg,#fbfcf8_0%,#f5f7ef_100%)] p-6">
      <PageTopbar title="User Permission List" breadcrumbs={["Admin", "User Permission"]} addLink="/admin/user-permission/form" />
          ...
          <div className="overflow-x-auto rounded-[26px] border border-[#ebe6d4] bg-white shadow-[0_24px_44px_rgba(47,60,24,0.08)]">
            <table className="w-full border-collapse text-[13px]">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={3} className="py-10 text-center text-ink-muted">Loading...</td>
                  </tr>
                ) : rows.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="py-10 text-center text-ink-muted">No records found</td>
                  </tr>
                ) : rows.map((r, i) => (
                  <tr key={r.unique_id} className="border-b border-[#edf1e4] odd:bg-[#fffefb] even:bg-[#f9fbf5] transition-colors hover:bg-[#f1f7e6]">
                    ...
                    <td className="px-4 py-4 font-medium text-[#243018]">{r.user_type}</td>
                    <td className="px-4 py-4">
                      <div className="flex gap-2">
                        <button onClick={() => navigate(`/admin/user-permission/form/${r.unique_id}`)} className="flex h-9 w-9 items-center justify-center rounded-2xl border border-blue-200 bg-info-light text-info transition-colors hover:bg-info hover:text-white">
