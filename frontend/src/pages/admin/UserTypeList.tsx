...
export default function UserTypeList() {
  ...
  const [rows, setRows] = useState<UserTypeListRow[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchUserTypeList({ search, start, length: requestLength });
      setRows(res.data);
      setTotal(res.recordsFiltered);
    } catch {
      setError("Failed to load records.");
      await showErrorAlert("Failed to load records.");
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    if (!confirmed) return;
    try {
      const res = await deleteUserType(unique_id);
      if (res?.status === 1 || res?.status === true) {
        await showSuccessAlert(res.msg ?? res.message ?? "User type deleted successfully.");
        await loadData();
        return;
      }
      await showErrorAlert(res?.error ?? res?.message ?? "Failed to delete user type.");
    } catch {
      await showErrorAlert("Failed to delete user type.");
    }
  };
  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(204,217,177,0.22),_transparent_30%),linear-gradient(180deg,#fbfcf8_0%,#f5f7ef_100%)] p-6">
      <PageTopbar
        ...
        breadcrumbs={["Admin", "User Type"]}
        addLink="/admin/user-type/form"
      />
            ...
            <table className="w-full border-collapse text-[13px]">
              <thead>
                <tr className="bg-[linear-gradient(135deg,#fcfbf6_0%,#eef3e3_100%)]">
                  {["#", "User Type", "Status", "Action"].map((h, i) => (
                    <th
                      key={i}
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <div className="flex gap-2">
                          <button
                            onClick={() => navigate(`/admin/user-type/form/${r.unique_id}`)}
