...
export default function CourierCreationList() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        setRows(await fetchCourierList());
      } catch {
  ...
  const filtered = useMemo(
    () =>
      rows.filter((row) =>
        `${row.courier_name}`.toLowerCase().includes(search.toLowerCase())
      ),
    [rows, search]
  );
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    try {
      const res = await deleteCourier(unique_id);
      if (res.status) {
  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(204,217,177,0.20),_transparent_30%),linear-gradient(180deg,#fbfcf8_0%,#f5f7ef_100%)] p-6">
      <PageTopbar
        title="Courier Creation List"
        breadcrumbs={["Settings", "Courier Creation"]}
        addLink="/settings/courier/form"
      />
            ...
            <table ref={tableRef} className="w-full text-[13px] border-collapse">
              <thead>
                <tr className="bg-[linear-gradient(135deg,#fcfbf6_0%,#eef3e3_100%)]">
                  {["#", "Courier Name", "Status", "Action"].map((h) => (
                    <th
                      key={h}
              ...
              <tbody>
                {loading ? (
                  ...
                  paged.map((row, index) => (
                    <tr key={row.unique_id} className="border-b border-[#edf1e4] odd:bg-[#fffefb] even:bg-[#f9fbf5] hover:bg-[#f1f7e6] transition-colors">
                      <td className="px-4 py-4 text-[#7a7f69]">
                        {(curPage - 1) * length + index + 1}
                      </td>
                      <td className="px-4 py-4 font-medium text-[#243018]">{row.courier_name}</td>
                      <td className="px-4 py-4">
                        <span
                          className={Number(row.is_active) === 1 || row.is_active === true ? "status-active" : "status-inactive"}
                        >
                          {row.is_active_display || (Number(row.is_active) === 1 || row.is_active === true ? "Active" : "Inactive")}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex gap-2">
                          <button
                            onClick={() => navigate(`/settings/courier/form/${row.unique_id}`)}
