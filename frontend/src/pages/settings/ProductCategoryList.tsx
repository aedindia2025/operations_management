...
export default function ProductCategoryList() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        setRows(await fetchProductCategoryList());
      } catch {
  ...
  const filtered = useMemo(
    () =>
      rows.filter((r) =>
        `${r.category_name} ${r.description ?? ""}`.toLowerCase().includes(search.toLowerCase())
      ),
    [rows, search]
  );
  ...
  const handleDelete = async (uniqueId: string) => {
    ...
    try {
      const res = await deleteProductCategory(uniqueId);
      const isSuccess = res.status === true || res.status === 1 || (!('status' in res) && !res.error);
      if (isSuccess) {
        setRows((prev) => prev.filter((row) => String(row.unique_id) !== uniqueId));
  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(204,217,177,0.20),_transparent_30%),linear-gradient(180deg,#fbfcf8_0%,#f5f7ef_100%)] p-6">
      <PageTopbar
        ...
        breadcrumbs={["Settings", "Product Category"]}
        addLink="/settings/product-category/form"
      />
            ...
            <table ref={tableRef} className="w-full text-[13px] border-collapse">
              <thead>
                <tr className="bg-[linear-gradient(135deg,#fcfbf6_0%,#eef3e3_100%)]">
                  {["#", "Product creation", "Description", "Status", "Action"].map((h, i) => (
                    <th
                      key={i}
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={r.unique_id} className="border-b border-[#edf1e4] odd:bg-[#fffefb] even:bg-[#f9fbf5] transition-colors hover:bg-[#f1f7e6]">
                    <td className="px-4 py-4 text-[#7a7f69]">{(curPage - 1) * length + i + 1}</td>
                    <td className="px-4 py-4 font-medium text-[#243018]">{r.category_name || "-"}</td>
                    <td className="px-4 py-4 text-[#4d5838]">{r.description || "-"}</td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex gap-2">
                        <button
                          onClick={() => navigate(`/settings/product-category/form/${r.unique_id}`)}
                        ...
                        <button
                          onClick={() => handleDelete(String(r.unique_id))}
