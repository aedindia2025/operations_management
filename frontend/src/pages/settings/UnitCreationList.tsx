...
import {
  deleteUnitCreation,
  fetchUnitCreationList,
  type UnitCreationRecord,
...
export default function UnitCreationList() {
  ...
  const [rows, setRows] = useState<UnitCreationRecord[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        setRows(await fetchUnitCreationList());
      } catch {
  ...
  const filtered = useMemo(
    () =>
      rows.filter((row) =>
        `${row.unit_name} ${row.description || ""}`.toLowerCase().includes(search.toLowerCase())
      ),
    [rows, search]
  );
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    try {
      const res = await deleteUnitCreation(unique_id);
      if (res.status) {
  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(204,217,177,0.20),_transparent_30%),linear-gradient(180deg,#fbfcf8_0%,#f5f7ef_100%)] p-6">
      <PageTopbar
        title="Unit Creation List"
        breadcrumbs={["Settings", "Unit Creation"]}
        addLink="/settings/unit/form"
      />
                ...
                {loading ? (
                  ...
                  paged.map((row, index) => (
                    <tr key={row.unique_id} className="border-b border-[#edf1e4] odd:bg-[#fffefb] even:bg-[#f9fbf5] transition-colors hover:bg-[#f1f7e6]">
                      ...
                      <td className="px-3 py-2 border border-line">{row.unit_name}</td>
                      ...
                      <td className="px-3 py-2 border border-line">
                        <div className="flex gap-2">
                          <button
                            onClick={() => navigate(`/settings/unit/form/${row.unique_id}`)}
