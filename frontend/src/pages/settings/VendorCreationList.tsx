...
export default function VendorCreationList() {
  const navigate = useNavigate();
  const tableRef = useRef<HTMLTableElement | null>(null);
  const [rows, setRows] = useState<VendorCreationRecord[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        setRows(await fetchVendorCreationList());
      } catch {
  ...
  const filtered = useMemo(
    () =>
      rows.filter((row) => {
        const value = `${row.vendor_id || ""} ${row.company_name} ${row.name} ${row.contact_no} ${row.mail_id || ""}`.toLowerCase();
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    try {
      const res = await deleteVendorCreation(unique_id);
      if (res.status) {
  ...
  return (
    <div className="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(204,217,177,0.20),_transparent_30%),linear-gradient(180deg,#fbfcf8_0%,#f5f7ef_100%)] p-6">
      <PageTopbar
        title="Vendor Creation List"
        breadcrumbs={["Settings", "Vendor Creation"]}
        addLink="/settings/vendor/form"
      />
      ...
      <div className="mt-4 overflow-hidden rounded-[30px] border border-[#e5e8d7] bg-white shadow-[0_24px_60px_rgba(46,61,24,0.08)]">
        <div className="p-5">
            ...
            <table ref={tableRef} className="w-full text-[13px] border-collapse">
              <thead>
                <tr className="bg-[linear-gradient(135deg,#fcfbf6_0%,#eef3e3_100%)]">
                  {["#", "Vendor ID", "Company Name", "Contact Person", "Contact No", "Status", "Action"].map((h) => (
                ...
                {loading ? (
                  ...
                  paged.map((row, index) => (
                    <tr key={row.unique_id} className="border-b border-[#edf1e4] odd:bg-[#fffefb] even:bg-[#f9fbf5] transition-colors hover:bg-[#f1f7e6]">
                      ...
                      <td className="px-3 py-2 border border-line">{row.vendor_id || "-"}</td>
                      <td className="px-3 py-2 border border-line">{row.company_name}</td>
                      <td className="px-3 py-2 border border-line">{row.name}</td>
                      <td className="px-3 py-2 border border-line">{row.contact_no}</td>
                      ...
                      <td className="px-3 py-2 border border-line">
                        <div className="flex gap-2">
                          <button
                            onClick={() => navigate(`/settings/vendor/form/${row.unique_id}`)}
          ...
          <div className="flex items-center justify-between mt-3 text-[13px] text-ink-secondary flex-wrap gap-2">
            ...
            <div className="flex gap-2">
              <button disabled={curPage === 1} onClick={() => setCurPage((p) => p - 1)} className="h-[36px] rounded-2xl border border-[#d8dec8] bg-white px-4 text-[13px] hover:border-[#7b8f43] hover:text-[#5f7427] disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">
                Previous
              </button>
              ...
              />
              <button disabled={curPage >= totalPages} onClick={() => setCurPage((p) => p + 1)} className="h-[36px] rounded-2xl border border-[#d8dec8] bg-white px-4 text-[13px] hover:border-[#7b8f43] hover:text-[#5f7427] disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">
                Next
              </button>
            </div>
          </div>
