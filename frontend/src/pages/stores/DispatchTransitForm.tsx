...
import {
  fetchDispatchTransitDetail,
  updateDispatchTransit,
  type DispatchTransitDetail,
} from "../../api/dispatchApi";
import { showErrorAlert, showSuccessAlert } from "../../utils/alerts";
import SearchableSelectInput from "../../components/common/SearchableSelectInput";

const emptyDetail: DispatchTransitDetail = {
  po_num: "",
  po_date: "",
  invoice_no: "",
  invoice_date: "",
...
export default function DispatchTransitForm() {
  ...
  const [detail, setDetail] = useState<DispatchTransitDetail>(emptyDetail);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!uniqueId || !consigneeId || !dcNo) {
        setLoading(false);
        await showErrorAlert("Transit record parameters are missing.");
        navigate("/stores/dispatch/list");
        return;
      }
      setLoading(true);
      try {
        const res = await fetchDispatchTransitDetail({
          unique_id: uniqueId,
          consignee_unique_id: consigneeId,
          dc_no: dcNo,
        });
        if (!active) return;
        if (!res.status) throw new Error(res.error || "Failed to load transit details.");
        setDetail(res.data);
        ...
        if (!active) return;
        await showErrorAlert(error instanceof Error ? error.message : "Failed to load transit details.");
        navigate("/stores/dispatch/list");
      } finally {
  ...
  const handleSave = async () => {
    if (!deliveryStatus) {
      await showErrorAlert("Please select delivery status.");
      return;
    }
    if (!deliveryDate) {
    ...
    if (!deliveryProof) {
      await showErrorAlert("Please attach delivery proof.");
      return;
    }

    const payload = new FormData();
    payload.append("delivery_status", deliveryStatus);
    ...
    payload.append("status", detail.delivery_status || "");
    ...
    payload.append("mode_of_delivery", detail.mode_of_delivery || "");
    payload.append("name_of_courier", detail.name_of_courier || "");
    payload.append("pod_no", detail.pod_no || "");
    payload.append("dispatch_date", detail.dispatch_date || "");
    payload.append("file", deliveryProof);
    if (podProof) payload.append("podfile", podProof);

    setSaving(true);
    try {
      const res = await updateDispatchTransit(payload);
      if (!res.status) throw new Error(res.error || res.message || "Failed to update transit details.");
      await showSuccessAlert("Transit details updated successfully.");
      navigate("/stores/dispatch/list");
    } catch (error) {
      await showErrorAlert(error instanceof Error ? error.message : "Failed to update transit details.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <div className="p-6 text-sm text-ink-secondary">Loading transit details...</div>;
  }

  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#f7f4e7_0%,#f3f6ee_32%,#eef2e8_100%)] p-6">
      <PageTopbar title="Dispatch" breadcrumbs={["Stores", "Dispatch", "Transit"]} />
      ...
      <div className="mt-4 overflow-hidden rounded-[30px] border border-[#e4e8d7] bg-white shadow-[0_24px_60px_rgba(46,61,24,0.12)]">
          ...
          <div>
            ...
            {[
              ...
              <div key={label} className="flex items-start gap-2 text-[13px] mb-1">
                ...
                <span className="text-ink-muted">:</span>
                <input name="dispatchtransitform_input_219"
                  value={value || ""}
                  readOnly
                  className="w-full border border-line-dark rounded px-2 py-1 text-[12px] bg-gray-50"
                />
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 p-6 md:grid-cols-2 xl:grid-cols-4">
          <div>
            ...
            <SearchableSelectInput name="deliverystatus"
          ...
          <div>
            <span className="mb-2 block text-[13px] font-semibold text-[#445131]">Delivery Proof Attachment</span>
            <input name="dispatchtransitform_input_252"
              type="file"
          ...
          <div>
            ...
            <input name="dispatchtransitform_input_261"
              type="file"
              accept="application/pdf,image/*"
