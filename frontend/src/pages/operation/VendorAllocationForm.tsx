...
import {
  createRevendorBulkAssign,
  createRevisitPaymentBulkAssign,
  ...
  fetchRevendorAllocationProductDetails,
  fetchRevisitPaymentPending,
  fetchRevisitPaymentProductDetails,
...
export default function VendorAllocationForm() {
  ...
  const isRevendor = window.location.pathname.includes("/revendor-allocation/");
  const isRevisitPayment = window.location.pathname.includes("/revisit-payment/") || window.location.pathname.includes("/revist-payment/");
  const pageTitle = isRevisitPayment ? "Revisit Payment" : isRevendor ? "Revendor Allocation" : "Vendor Allocation";
  const listPath = isRevisitPayment ? "/vendor/revisit-payment/list" : isRevendor ? "/operation/revendor-allocation/list" : "/operation/vendor-allocation/list";
  ...
  useEffect(() => {
      ...
      try {
        if (recordId) {
          ...
          const [meta, pending] = await Promise.all([
            fetchVendorAllocationMeta(),
            isRevisitPayment
              ? fetchRevisitPaymentPending({ page: 1, length: 1000, search: "", skip_count: 1 })
              : isRevendor
              ? fetchRevendorAllocationPending({ page: 1, length: 1000, search: "", skip_count: 1 })
    ...
    loadForm();
  }, [recordId, isRevendor, isRevisitPayment]);
  ...
  useEffect(() => {
    const loadProductRows = async () => {
      ...
      try {
        const response = await (isRevisitPayment ? fetchRevisitPaymentProductDetails : isRevendor ? fetchRevendorAllocationProductDetails : fetchVendorAllocationProductDetails)({
          po_id: poId,
    ...
    loadProductRows();
  }, [engineerType, filteredBySelection, gstType, selectedInvoice, isRevendor, isRevisitPayment]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const toggleRow = (rowId: string) => {
    ...
    setRows((prev) => {
      ...
      if (nextSelected) {
        if (!isRevendor && !isRevisitPayment && (engineerType === "own-engineer" || engineerType === "inhouse") && target.partialSts === "1") {
          setError("Partial DC rows cannot be assigned to own engineer or inhouse team.");
          return prev;
        }

        if (!isRevendor && !isRevisitPayment && engineerType === "outsource-vendor" && selectedRows.length > 0 && (hasExistingPartial || target.partialSts === "1")) {
  ...
  const toggleAll = (checked: boolean) => {
    ...
    setRows((prev) => prev.map((row) => {
      ...
      if (!isRevendor && !isRevisitPayment && (engineerType === "own-engineer" || engineerType === "inhouse") && row.partialSts === "1") {
        return { ...row, selected: false };
      }
      if (!isRevendor && !isRevisitPayment && engineerType === "outsource-vendor" && row.partialSts === "1") {
        return { ...row, selected: false };
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const saveVendorAssign = isRevisitPayment ? createRevisitPaymentBulkAssign : isRevendor ? createRevendorBulkAssign : createVendorBulkAssign;
      const res = await saveVendorAssign({
      ...
      if (res.status) {
        await showSuccessAlert(isRevisitPayment ? "Revisit payment saved successfully." : isRevendor ? "Revendor allocation saved successfully." : "Vendor allocation saved successfully.");
        navigate(listPath);
      } else {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#f6f7ea_0%,#f9f7ef_35%,#f5f5f0_100%)] p-4 md:p-6">
      <PageTopbar title={pageTitle} breadcrumbs={[isRevisitPayment ? "Vendor Payment" : "Operation", pageTitle, recordId ? "Edit" : "Add"]} />
