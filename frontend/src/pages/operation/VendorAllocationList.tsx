...
import {
  ...
  fetchRevendorAllocationPending,
  fetchRevisitPaymentPending,
...
export default function VendorAllocationList() {
  ...
  const isRevendor = location.pathname.includes("/revendor-allocation/");
  const isRevisitPayment = location.pathname.includes("/revisit-payment/") || location.pathname.includes("/revist-payment/");
  const pageTitle = isRevisitPayment ? "Revisit Payment List" : isRevendor ? "Revendor Allocation List" : "Vendor Allocation List";
  const breadcrumbs = isRevisitPayment ? ["Vendor Payment", "Revisit Payment"] : ["Operation", isRevendor ? "Revendor Allocation" : "Vendor Allocation"];
  const formPath = isRevisitPayment ? "/vendor/revisit-payment/form" : isRevendor ? "/operation/revendor-allocation/form" : "/operation/vendor-allocation/form";
  const tabs = isRevendor || isRevisitPayment ? (["pending"] as const) : (["pending", "completed"] as const);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (activeTab === "pending") {
          const fetchPending = isRevisitPayment
            ? fetchRevisitPaymentPending
            : isRevendor
              ? fetchRevendorAllocationPending
              : fetchVendorAllocationPending;
          const res = await fetchPending(params);
    ...
    load();
  }, [activeTab, search, length, curPage, fromDate, toDate, dateType, teamMember, isRevendor, isRevisitPayment]);
  ...
  useEffect(() => {
    if ((isRevendor || isRevisitPayment) && activeTab !== "pending") {
      setActiveTab("pending");
    }
  }, [activeTab, isRevendor, isRevisitPayment]);
  ...
  const pageStart = total === 0 ? 0 : (curPage - 1) * length + 1;
  const pageEnd = Math.min(curPage * length, total);

  const pageNums = useMemo(() => getPaginationItems(curPage, totalPages), [curPage, totalPages]);
  const showCreateButton = activeTab === "pending" && !isRevendor;
  const createButtonLabel = isRevisitPayment ? "Revisit Payment" : "Vendor Allocation";

  const handleExportApi = async () => {
    try {
      const type = activeTab === "pending" ? "pending" : "complete";
      const res = await exportVendorAllocationData(type);
        ...
        <div className="p-5">
          ...
          <div className="flex items-end gap-3 mb-5 flex-wrap">
            ...
            <div>
              ...
              <SearchableSelect
                name="vendor_allocation_date_type"
                value={dateType}
            ...
            <div>
              ...
              <SearchableSelect
                name="vendor_allocation_team_member"
                value={teamMember}
          ...
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2 text-[13px] text-ink-secondary">
              Search:
              <input name="vendor_allocation_search" value={search} placeholder="Search..." onChange={(e) => setSearch(e.target.value)} className="px-2.5 py-1 text-[13px] border border-line-dark rounded outline-none w-48 focus:border-brand-500 focus:ring-1 focus:ring-brand-500/20" />
            </div>
          </div>
                ...
                {loading ? (
                  ...
                  <tr key={r.id} className="hover:bg-brand-50/40 transition-colors border-b border-line/50">
                    ...
                    <td className="px-3 py-3 text-center border-x border-line/50">
                      <button onClick={() => navigate(`${formPath}/${r.id}`)} title={activeTab === "pending" ? "Assign" : "Edit"} className="w-7 h-7 flex items-center justify-center rounded bg-info-light text-info border border-blue-200 text-[13px] hover:bg-info hover:text-white transition-colors cursor-pointer mx-auto">
          ...
          <div className="flex items-center justify-between mt-3 text-[13px] text-ink-secondary flex-wrap gap-2">
            ...
            <div className="flex gap-1">
              <button disabled={curPage === 1} onClick={() => setCurPage((p) => p - 1)} className="px-3 h-[30px] text-[13px] bg-white border border-line rounded hover:border-brand-500 hover:text-brand-500 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">Previous</button>
              ...
              />
              <button disabled={curPage >= totalPages} onClick={() => setCurPage((p) => p + 1)} className="px-3 h-[30px] text-[13px] bg-white border border-line rounded hover:border-brand-500 hover:text-brand-500 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">Next</button>
            </div>
          </div>
        </div>
      </div>
