...
const API = "/api/master/management-bill-approval";
...
interface Bill {
  ...
  mgmt_approved_by: string;
  mgmt_approved_date: string;
  remarks: string;
  status: string;
}
...
function resolveAttachmentUrl(rawUrl?: string, folder = "vendorpayment") {
  const raw = String(rawUrl ?? "").trim();
  if (!raw || raw === "#") return "";
  if (/^https?:\/\//i.test(raw)) return raw;
  if (raw.startsWith("/")) return raw;
  const { protocol, hostname } = window.location;
  return `${protocol}//${hostname}/otm_beta/uploads/${folder}/${raw}`;
}

function ModalWrapper({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-start justify-center overflow-y-auto p-4 md:p-6" onClick={onClose}>
      <div className="bg-white w-full max-w-[92vw] rounded-[18px] shadow-2xl overflow-hidden border border-line max-h-[96vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div
        ...
        >
          <h2 className="absolute inset-x-0 pointer-events-none text-center text-[15px] font-bold text-[#5d6a1a]">{title}</h2>
          <button
            type="button"
            onClick={onClose}
            className="relative z-10 w-8 h-8 flex items-center justify-center rounded-full bg-[#c0392b] text-white hover:bg-[#a93226] transition-colors text-[18px] leading-none"
          >
            &times;
          </button>
        </div>
...
function HeaderCard({ detail, onOpenFile }: { detail: BillDetail; onOpenFile: (url: string) => void }) {
  return (
...
export default function ManagementTeamBillApproval() {
  ...
  const openModal = async (type: "edit" | "view", bill_no: string) => {
    ...
    setDetail({
      ...data,
      invoice_attach_url: resolveAttachmentUrl(data.invoice_attach_url, "vendorpayment"),
      po_attach_url: resolveAttachmentUrl(data.po_attach_url, "vendorpayment"),
      pan_copy_url: resolveAttachmentUrl(data.pan_copy_url, "vendor_creation"),
      bank_proof_url: resolveAttachmentUrl(data.bank_proof_url, "vendor_creation"),
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API}/approve/`, {
        ...
        body: JSON.stringify({ bill_no: modal.bill_no, status: approvalStatus, remark: remarks }),
      });
      if (!response.ok) throw new Error();
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#f7f4e7_0%,#f3f6ee_32%,#eef2e8_100%)] p-6">
      <PageTopbar title="Management Team Bill Approval" breadcrumbs={["Vendor Payment", "Management Team Bill Approval"]} />
      <VendorPaymentPageIntro
        title="Management Team Bill Approval"
        description="Finalize management-level approvals after accounts review with a clearer, modern layout while preserving the same approval flow and actions."
        metrics={[
      ...
      <div className="mt-4 overflow-hidden rounded-[30px] border border-[#e2e7d0] bg-white shadow-[0_24px_60px_rgba(46,61,24,0.12)]">
        <PageTabs
          items={[
            { value: "Pending", label: "Management Team Bill Pending" },
            { value: "Complete", label: "Management Team Bill Complete" },
          ]}
          value={tab}
          onChange={(value) => {
            setTab(value);
          ...
          <div className="mb-4 flex items-center justify-between gap-3 rounded-[28px] border border-[#ecd9a2] bg-[linear-gradient(135deg,#fffdf6_0%,#fffaf2_44%,#f9fbef_100%)] px-5 py-4 shadow-[0_18px_36px_rgba(104,116,40,0.07)] flex-wrap">
            <div className="flex items-center gap-2 text-[13px] text-ink-secondary flex-wrap">
              ...
              {["Copy", "CSV", "Excel", "PDF", "Print"].map((btn) => (
                <button
                  key={btn}
                  onClick={() => { if (btn === "Excel") window.location.href = `${API}/export/`; }}
            ...
            <table className="w-full text-[12px] border-collapse">
              <thead>
                <tr className="bg-surface-2">
                  {(tab === "Pending"
                    ? ["S.No", "Vendor Bill Date / Bill No", "Vendor Invoice Date / Invoice No", "Vendor Name", "DC Count", "Amount", "TDS Deduction", "Other Deduction", "Advance Amount", "Total Payable Amount", "Vendor Approved By / Date", "Accounts Approved By / Date", "Remarks", "Action"]
                    : ["S.No", "Vendor Bill Date / Bill No", "Vendor Invoice Date / Invoice No", "Vendor Name", "DC Count", "Amount", "TDS Deduction", "Other Deduction", "Advance Amount", "Total Payable Amount", "Management Team Approved By / Date", "Status", "Action"]
                  ).map((header) => <th key={header} className="border border-line-dark px-3 py-2 text-left whitespace-nowrap">{header}</th>)}
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={14} className="text-center py-8 text-ink-muted text-[12px]">Loading...</td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={14} className="text-center py-8 text-ink-muted text-[12px]">No data available in table</td></tr>
                ) : (
                  rows.map((bill, index) => (
                    <tr key={bill.id} className="hover:bg-brand-50/40 border-b border-line/50">
                      ...
                      <td className="px-3 py-2 border max-w-[260px]">{bill.vendor_name || "-"}</td>
                      ...
                      <td className="px-3 py-2 text-right border">{fmtAmt(bill.other_deduction)}</td>
                      ...
                      {tab === "Pending" ? (
                        <>
                          ...
                          <td className="px-3 py-2 border"><div className="font-medium">{bill.accounts_approved_by || "-"}</div><div className="text-ink-muted text-[11px]">{bill.accounts_approved_date || "-"}</div></td>
                          <td className="px-3 py-2 border">{bill.remarks || "-"}</td>
                        </>
                      ) : (
                        <>
                          <td className="px-3 py-2 border"><div className="font-medium">{bill.mgmt_approved_by || "-"}</div><div className="text-ink-muted text-[11px]">{bill.mgmt_approved_date || "-"}</div></td>
      ...
      {modal?.type === "edit" && (
        <ModalWrapper title="Management Team Bill Approval" onClose={closeModal}>
          {detailLoading || !detail ? (
            <div className="text-center py-8 text-ink-muted">Loading...</div>
          ) : (
            <>
              <HeaderCard detail={detail} onOpenFile={openFile} />
              ...
              <div className="rounded-lg border border-line bg-white p-5 shadow-[0_1px_4px_rgba(15,23,42,0.04)]">
                ...
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_290px] items-start">
                  <div className="space-y-6">
                    ...
                    <div className="max-w-[480px]">
                      ...
                      <SearchableSelectInput name="approvalstatus" value={approvalStatus} onChange={(e) => setApprovalStatus(e.target.value)} className="border border-line-dark rounded px-3 py-2 text-[12.5px] w-full focus:outline-none focus:border-brand-500">
                    ...
                    {detail.reject_reason && (
                      <div className="max-w-[480px]">
                        ...
                        <div className="border border-line rounded px-3 py-2 text-[12.5px] text-ink-secondary bg-surface-2">{detail.reject_reason}</div>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex justify-end gap-3 mt-6">
                  <button type="button" onClick={closeModal} className="px-5 py-2 rounded text-[13px] font-semibold bg-danger text-white hover:bg-red-700">Close</button>
                  <button type="button" disabled={submitting} onClick={handleSubmit} className="px-5 py-2 rounded text-[13px] font-semibold bg-brand-700 text-white hover:bg-brand-800 disabled:opacity-60">{submitting ? "Saving..." : "Submit"}</button>
                </div>
              </div>
            </>
          )}
      ...
      {modal?.type === "view" && (
        <ModalWrapper title="Management Team Approval" onClose={closeModal}>
          {detailLoading || !detail ? (
            <div className="text-center py-8 text-ink-muted">Loading...</div>
          ) : (
            <>
              ...
              <div className="rounded-lg border border-line bg-white p-5 shadow-[0_1px_4px_rgba(15,23,42,0.04)]">
                ...
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_290px] items-start mb-6">
                  <div className="pt-2 text-[13px] text-ink-secondary flex flex-wrap gap-x-6 gap-y-2">
                    <span>
                      Management Team Bill approved By: <strong className="text-[#647026]">{displayText(detail.approvals?.[0]?.management_by)}</strong>
                    </span>
                    <span>
                      Management Team Bill approved Date: <strong className="text-[#647026]">{displayText(detail.approvals?.[0]?.management_at).split(" ")[0] || "-"}</strong>
                    </span>
                  </div>
